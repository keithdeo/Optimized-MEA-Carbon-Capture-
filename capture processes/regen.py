...
def water_evaporation(co2_desorbed_mol_s, stripping_water_per_co2):
    ...
    water_evaporated_kg_s = (
        water_evaporated_mol_s * WATER_MOLAR_MASS_G_MOL / 1000.0
    )
...
def reboiler_duty(
    ...
    sensible_heat_kW = (
        solvent_mass_flow * cp * (regeneration_temp - feed_temp)
    )
    desorption_heat_kW = co2_desorbed * heat_desorption / 1000.0

    water_evaporated_mol_s, water_evaporated_kg_s = water_evaporation(
        co2_desorbed, stripping_water_per_co2
    )

    evaporation_heat_kW = (
        water_evaporated_kg_s * LATENT_HEAT_WATER_J_KG / 1000.0
    )
    total_heat_kW = (
        sensible_heat_kW + desorption_heat_kW + evaporation_heat_kW
    )
...
def simulate_regeneration(
    ...
    co2_desorbed_mol_s = co2_desorbed(
        mea_molar_flow_mol_s, rich_loading, lean_loading
    )
