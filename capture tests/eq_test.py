...
if __name__ == "__main__":
    print("=== MEA-CO2 EQUILIBRIUM TEST ===")

    try:
        t_celsius = float(input("Enter temperature (°C): "))
        total_co2 = float(input("Enter total CO2 concentration (mol/L): "))
        total_mea = float(input("Enter total MEA concentration (mol/L): "))
        h_conc = 1e-9  # Fixed sample H+ concentration

        t_kelvin = user_temp(t_celsius)
        constants = equilibrium_constants(t_kelvin)

        print("\nEquilibrium Constants:")
        for name, value in constants.items():
            print(f"  {name}: {value:.4e}")

        species = equilibrium_state(t_kelvin, total_co2, total_mea, h_conc)

        print("\nSpecies Concentrations:")
        for name, concentration in species.items():
            print(f"  {name:8s}: {concentration:.6e} mol/L")
