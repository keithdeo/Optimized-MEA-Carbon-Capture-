...
def simulate_process(
    ...
    for iteration in range(1, iterations + 1):
        ...
        # 1. Absorber step using current guessed lean_loading
        ...
        # 2. Regeneration step: pass lean_loading=None so regen.py calculates 
        ...
        # 3. Calculate target stripper exit loading incorporating column effectiveness
        ...
        # 4. Apply damped feedback to compute next iteration's lean loading
...
if __name__ == "__main__":
    print("        CO2 CAPTURE PROCESS TEST      ")

    results = simulate_process(
        co2_inlet_fraction=0.12,
        gas_flow=35.0,              # Corrected: 35.0 mol/s gas (~1.0 kg/s flue gas)
        solvent_flow=3.0,            # 3.0 kg/s solvent (14.73 mol/s MEA)
        mea_mass_fraction=0.30,
        absorber_temp=313.15,
        pressure_Pa=101325.0,
        column_height=15.0,
        column_area=1.0,
        mass_coeff=150.0,
        specific_interfacial=250.0,
        regeneration_temp=393.15,
        feed_temp=378.15,
        initial_loading=0.20,
        cp=3.5,
        heat_desorption=85000.0,
        stripping_water_per_co2=0.8,
        reboiler_efficiency=0.85,
        regeneration_effectiveness=0.95,
        damping_factor=0.25,
        iterations=1000,
        convergence_tolerance=1e-4,
        co2_balance_tolerance=0.05
    )

    print("            PROCESS RESULTS             ")
    print(f"\nConverged:               {results['converged']}")
    print(f"Iterations:              {results['iterations']}")
    print(f"Lean loading:            {results['lean_loading']:.4f} mol/mol")
    print(f"Rich loading:            {results['rich_loading']:.4f} mol/mol")
    print(f"CO2 capture:             {results['capture_percentage']:.2f}%")
    print(f"Outlet CO2 fraction:     {results['outlet_co2_fraction']:.6f}")
    print(f"CO2 absorbed:            {results['co2_absorbed_mol_s']:.6f} mol/s")
    print(f"CO2 desorbed:            {results['co2_desorbed_mol_s']:.6f} mol/s")
    print(f"Reboiler duty:           {results['reboiler_duty_kW']:.4f} kW")
    print(f"Specific energy:         {results['specific_energy_J_per_kg']:.4f} J/kg CO2 ({results['specific_energy_GJ_per_tonne_CO2']:.4f} GJ/tonne CO2)")
    print(f"Sensible heat:           {results['sensible_heat_kW']:.4f} kW")
    print(f"Desorption heat:         {results['desorption_heat_kW']:.4f} kW")
    print(f"Evaporation heat:        {results['evaporation_heat_kW']:.4f} kW")